...
pool.getConnection(function(err,connection){ //从连接池中获取一个连接
   if(err){
       console.info("获取数据库连接失败...."+err);
        ...
        //左外连接：以left join左边的表为基表，基表中所有的数据都显示，非基本表没有的用 null 补空    right join：右外连接   full join:完全外连接  inner join：内连接
        ...
        //查询第1页，每页2条 limit 0,2   查询第2页，每页2条记录  limit 2,2  查询第3页，每页2条  limit 4,2   查询第n页，每页m条  limit (n-1)*m,m
